...
int _kEDAMErrorCodeValues[] = {
  ...
  EDAMErrorCode::SSO_AUTHENTICATION_REQUIRED
};
...
const std::map<int, const char*> _EDAMErrorCode_VALUES_TO_NAMES(::apache::thrift::TEnumIterator(28, _kEDAMErrorCodeValues, _kEDAMErrorCodeNames), ::apache::thrift::TEnumIterator(-1, NULL, NULL));

std::ostream& operator<<(std::ostream& out, const EDAMErrorCode::type& val) {
  std::map<int, const char*>::const_iterator it = _EDAMErrorCode_VALUES_TO_NAMES.find(val);
  if (it != _EDAMErrorCode_VALUES_TO_NAMES.end()) {
    out << it->second;
  } else {
...
std::ostream& operator<<(std::ostream& out, const EDAMInvalidContactReason::type& val) {
  std::map<int, const char*>::const_iterator it = _EDAMInvalidContactReason_VALUES_TO_NAMES.find(val);
  if (it != _EDAMInvalidContactReason_VALUES_TO_NAMES.end()) {
    out << it->second;
  } else {
    out << static_cast<int>(val);
  }
